...
namespace {
    ...
    constexpr double kTropoExp =
        kGravity * kMolarMassDryAir / (kUniversalGas * kLapseRate); // ~5.2561
} // anonymous namespace
...
double compute_air_density(double temperature_K,
                           double pressure_Pa,
                           double relative_humidity) noexcept
{
                                     ...
                                     const Wind& wind) noexcept
{
    ...
    if (altitude_m <= kTropopause) {
        ...
        const double P = surface_pressure_Pa *
                         std::pow(T / surface_temp_K, kTropoExp);
        ...
        const double P11_adj = surface_pressure_Pa *
                               std::pow(T11_adj / surface_temp_K, kTropoExp);

        const double T = kT11;
        const double P = P11_adj * std::exp(
            -kGravity * kMolarMassDryAir * (altitude_m - kTropopause) /
            (kUniversalGas * kT11));
        cond.temperature_K = T;
        cond.pressure_Pa   = P;
    } else {
        ...
        cond.pressure_Pa   = P11_adj * std::exp(
            -kGravity * kMolarMassDryAir * (kStratoPause - kTropopause) /
            (kUniversalGas * kT11));
    }
